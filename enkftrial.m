% Script for running trials of the EnKF on the synthetic test case
...
for trial = 1:numtrials
    ...
    for time_step = 1:nsteps
        ...
        H = zeros(4,2*tlen+(4*nsurfs+1)*prod(dims));
        for i = 1:4
            H(i,obs_indices(i)) = 1;
        end
        Y = data.perturbed{time_step};
        HXf = Xf(obs_indices,:);
        HCXfHt = cov(HXf');
    ...
    % Copy dfu and sed files back into working directory
    copyfile('../templates/mkref.dfu.json','.');
    copyfile('../templates/mkref.sed.json','.');
    
    % Save results of individual trial
